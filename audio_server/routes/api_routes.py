...
@router.post("/audio/", response_description="Adding audiofile....")
...
async def get_audio_data(id):
    ...
    return ErrorResponseModel("An error occurred.", "audio   exist.")
...
async def update_audio_data(id: str, song: UpdateSongModel = None,podcast: UpdatePodcastModel = None,
    ...
    if song:
        ...
        if updated_podcast:
            return ResponseModel(
                "Podcast with ID: {}  update is successful".format(id),
        ...
        if updated_audiobook:
            return ResponseModel(
                "Audiobook with ID: {}  update is successful".format(id),
...
@router.delete("/audio/{id}", response_description="Audio data deleted from the database")
